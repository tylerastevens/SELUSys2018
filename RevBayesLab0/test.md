...
***A Simple Example for Building a Hierarchical Model and Sampling under
...
of `RevBayes`[@Hoehna2017a], which is available for Mac OS X, Windows,
and Linux operating systems. Directions for downloading and installing
the software are available on the program webpage:
[http://revbayes.com](http://revbayes.com/).

The exercise provided also requires additional programs for editing text
files and visualizing output. The following are very useful tools for
working with `RevBayes`:
...
\end{minipage}}
[\[fig:archery\_model\]]{#fig:archery_model label="fig:archery_model"}
...
a target
(Fig. [\[fig:archery\_model\]](#fig:archery_model){reference-type="ref"
reference="fig:archery_model"}). The distance $d$ of each arrow from the
target's center is measured. Let's assume that the distance of each
arrow from the bullseye follows an exponential
distribution---*i.e.,*$d\sim\mbox{Exp}(\mu^{-1})$. This implies the
archer has an inherent ability to shoot arrows at an average distance
$\mu$. Then, the probability density of each arrow distance $d_i$ is
$$\begin{aligned}
...
$\bar d$, $P(\mu \mid \bar d)$, is: $$\begin{aligned}
\overbrace{P(\mu \mid \bar d)}^{\text{posterior distribution}} = \frac{ \overbrace{P(\bar d \mid \mu)}^{\text{likelihood}} \times \overbrace{P(\mu)}^{\text{prior}}}{\underbrace{P(\bar d)}_{\text{marginal likelihood}}}\end{aligned}$$
...
arrow distance
(Figure [\[fig:exponential\_distribution\]](#fig:exponential_distribution){reference-type="ref"
reference="fig:exponential_distribution"}). Different choices for
$\alpha$ represent different prior beliefs.
...
\end{minipage}}
[\[fig:exponential\_distribution\]]{#fig:exponential_distribution
label="fig:exponential_distribution"}

Figure [\[fig:archery\_model\]](#fig:archery_model){reference-type="ref"
reference="fig:archery_model"} shows the graphical model for the archery
...
that this variable is constant (*i.e.,* it takes a "known" value).
Following the graph in
figure [\[fig:archery\_model\]](#fig:archery_model){reference-type="ref"
reference="fig:archery_model"}, we see an arrow connecting $\alpha$ and
...
characters given as []{style="background-color: shadecolor"}
differently. Thus, if you copy and paste text from this PDF, you may
introduce some incorrect characters. Because of this, we recommend that
you type the instructions in this tutorial or copy them from the scripts
provided
...
empty script in your text editor and follow each step provided as
[]{style="background-color: shadecolor"} below.
...
algorithm [@Metropolis1953; @Hastings1970] proceeds as follows:
...
2.  Propose a new value (which we'll call $\mu^\prime$) for some
    ...
    [\[sect:MH:step2\]]{#sect:MH:step2 label="sect:MH:step2"}
...
3.  Calculate the acceptance probability, $R$, according to:
    $$\begin{aligned}
    	...
    	\end{aligned}$$[\[sect:MH:step3\]]{#sect:MH:step3
    label="sect:MH:step3"}
...
4.  Generate a uniform random number $u$ between 1 and 0.
    ...
    [\[sect:MH:step4\]]{#sect:MH:step4 label="sect:MH:step4"}

5.  Record the values of the
    parameters.[\[sect:MH:step5\]]{#sect:MH:step5 label="sect:MH:step5"}

6.  Return to step 2 many, many times, keeping track of the value of
    $\mu$.[\[sect:MH\_alg:step6\]]{#sect:MH_alg:step6
    label="sect:MH_alg:step6"}
...
model perform---*i.e.,* how robust and accurate are our estimators.
After completing this exercise, feel free to repeat it and alter the
...
In `Rev`, we can create a *user-defined function* using the `function`
keyword. In our function definition above, `likelihood()` takes a single
value as an argument that is expected to be the mean ($\mu$) value. All
other parameters in our function are expected to be defined before
...
Additionally, we are going to monitor, *i.e.,* store, parameter values
into a file during the MCMC simulation. For this file we need to write
the column headers in the first line of our output file, which we will
name `archery_MH.log` (you may have to change the newline characters
from `"\backslashn"` to `"\backslashr\backslashn"` if you're using a
...
often we print to file (*i.e.,* monitor); this is called thinning if we
do not choose to save every value of our parameter to file. If we set
the variable `printgen=1`, then we will store the parameter values at
every iteration; if we instead choose `printgen=10`, then we'll only
save the values every $10^{th}$ step in our Markov chain.
...
[\[sect:MH\_alg:step6\]](#sect:MH_alg:step6){reference-type="ref"
reference="sect:MH_alg:step6"} in
Sect. [3.3](#sect:MH_algorithm){reference-type="ref"
reference="sect:MH_algorithm"}). We will start this part by defining the
number of iterations for our MCMC (`reps = 10000`) and writing the first
line of our `for` loop. We'll also define a variable `delta` (explained
momentarily).
...
of 'curly braces' `{...}`. Our loop will not be complete until we finish
it and add the closing brace. Additionally, it is good style to make our
loops readable by indenting the contents within the `{...}`. We
recommend that you use 4 spaces to represent these indents.

For our MCMC algorithm, the first thing we do is generate a new value of
$\mu^\prime$ to evaluate (step
[\[sect:MH:step2\]](#sect:MH:step2){reference-type="ref"
reference="sect:MH:step2"} of
Sect. [3.3](#sect:MH_algorithm){reference-type="ref"
reference="sect:MH_algorithm"}). We'll propose a new value of $\mu$ by
drawing a random number from a uniform window and then adding this
random number to the current value (*i.e.,* centered on the previous
value). The value of `delta` defines the width of the uniform window
...
value of $\mu$. Conversely, if `delta` is small, then the proposed
values are more likely to be very close to the current value of $\mu$.
By changing the value of `delta` we can tune the behavior of the
proposal, and therefore `delta` is called a *tuning parameter*.
...
$R$ (step [\[sect:MH:step3\]](#sect:MH:step3){reference-type="ref"
reference="sect:MH:step3"} of
Sect. [3.3](#sect:MH_algorithm){reference-type="ref"
reference="sect:MH_algorithm"}):
...
(step [\[sect:MH:step4\]](#sect:MH:step4){reference-type="ref"
reference="sect:MH:step4"} of
Sect. [3.3](#sect:MH_algorithm){reference-type="ref"
reference="sect:MH_algorithm"}):
...
[\[sect:MH:step5\]](#sect:MH:step5){reference-type="ref"
reference="sect:MH:step5"} of
Sect. [3.3](#sect:MH_algorithm){reference-type="ref"
reference="sect:MH_algorithm"}). Here, we actually check if we want to
store the value during this iteration.
...
Now that you have defined your model and written functions to compute
the probability and sample values of $\mu$, you are now ready to run
your analysis.

Begin by running the `RevBayes` executable. You can do this by
navigating to the folder containing your `RevBayes` executable and
running it. If you're on a Unix system you can do this by typing:

::: {style="background-color: black"}
\hspace{1mm}
[`./rb`]{style="color: 0,1,1"}
:::

*Alternatively*, if you are on a Unix system and the `RevBayes` binary
is in your path, you only have to type the following from any directory:
...
1.  Write and execute the script outlined above, which you can give any
    name you like (there is also an example file called
    `archery_MH.Rev`).

2.  The `.log` file will contain samples from the posterior distribution
    of the model. Open the file in `Tracer` to learn about various
    features of the posterior distribution, for example: the posterior
    mean or the 95% credible interval.

Pretty awesome, right?

Below we show an example of the obtained output in `Tracer`.
Specifically,
Figure [\[fig:mcmc\_samples\]](#fig:mcmc_samples){reference-type="ref"
reference="fig:mcmc_samples"} shows the sample trace (left) and the
estimated posterior distribution of $\mu$ (right). There are other
parameters, such as the posterior mean and the 95% HPD (highest
posterior density) interval, that you can obtain from `Tracer`.
...
\end{minipage}}
[\[fig:mcmc\_samples\]]{#fig:mcmc_samples label="fig:mcmc_samples"}
...
[**[!]{.sans-serif}**]{style="background-color: SpringGreen"} The
sliding-window and scaling moves are very common and popular moves in
`RevBayes`. The code examples here are actually showing the exact same
equation as implemented internally. It will be very useful for you to
...
[**[?]{.sans-serif}**]{style="background-color: Apricot"} Do the
posterior distributions for the parameter $mu$ look the same as the ones
...
0.001). As you can see in Figure
[\[fig:gamma\_distribution\]](#fig:gamma_distribution){reference-type="ref"
reference="fig:gamma_distribution"}, compared to the exponential
distribution, the Gamma(0.001, 0.001) distribution is much more "flat".
...
\end{minipage}}
[\[fig:gamma\_distribution\]]{#fig:gamma_distribution
